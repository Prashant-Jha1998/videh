    ...
    <li>
      <button
        ...
        className={`w-full flex items-start gap-3 rounded-lg px-3 py-2.5 text-left transition-colors ${
          status === "current"
            ? "bg-[#5B4FE8]/20 ring-1 ring-[#5B4FE8]/40"
            : status === "locked"
      ...
      >
        <span
          className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
            status === "current" ? "bg-[#5B4FE8] text-white" : "bg-white/10"
          }`}
        >
          {status === "done" ? (
            <CheckCircle2 className="h-4 w-4 text-[#5B4FE8]" />
          ) : status === "locked" ? (
...
export function OnboardingConsoleLayout({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex bg-[#e8ecef]">
      <aside className="hidden md:flex w-[280px] shrink-0 flex-col bg-[#14131F] text-white border-r border-white/10">
        <div className="p-5 border-b border-white/10">
          ...
          {reference ? (
            <p className="mt-3 text-xs font-mono text-[#5B4FE8] bg-[#5B4FE8]/10 px-2 py-1 rounded-md truncate">{reference}</p>
          ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0">
        <header className="shrink-0 h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 gap-3">
          <div className="flex items-center gap-3 min-w-0 md:hidden">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#5B4FE8]/10 p-1">
              <img src="/videh_icon_foreground.png" alt="" className="h-full w-full object-contain" />
            </span>
            <span className="font-semibold text-sm text-[#14131F] truncate">Videh API Console</span>
          </div>
          <div className="hidden md:block min-w-0">
            <p className="text-xs text-[#667781] font-medium uppercase tracking-wide">Module</p>
            <h1 className="text-lg font-bold text-[#14131F] truncate">{currentModule.label}</h1>
          </div>
          <div className="flex items-center gap-2 ml-auto">
            {reference ? (
              <span className="hidden sm:inline text-xs font-mono text-[#5B4FE8] bg-[#5B4FE8]/10 px-2 py-1 rounded-md">{reference}</span>
            ) : null}
            <button type="button" onClick={onClose} className="text-[#667781] hover:text-[#14131F] p-2 rounded-lg hover:bg-gray-100" aria-label="Exit console">
              <X className="h-5 w-5" />
            </button>
          </div>
        </header>
            ...
            {modules.map((mod) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap ${
                  step === mod.id ? "bg-[#5B4FE8] text-white" : !canGoTo(mod.id) ? "bg-gray-100 text-gray-400" : "bg-gray-100 text-[#14131F]"
                }`}
              >
                {mod.label}
              </button>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8" data-console-main>
          <div className={fullWidth ? "w-full max-w-none" : "max-w-3xl mx-auto w-full"}>
            <div className="md:hidden mb-4">
              <h1 className="text-xl font-bold text-[#14131F]">{currentModule.label}</h1>
              <p className="text-sm text-[#667781]">{currentModule.subtitle}</p>
            </div>
            {!submitted && progressSteps.length > 0 ? (
              <div className="flex gap-1 mb-6">
                {progressSteps.map((s, i) => (
                  <div key={s} className={`h-1 flex-1 rounded-full ${i <= stepIndex ? "bg-[#5B4FE8]" : "bg-gray-200"}`} />
                ))}
              </div>
            ) : null}
            {children}
