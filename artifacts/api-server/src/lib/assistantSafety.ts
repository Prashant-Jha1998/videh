...
const REFUSAL: Partial<Record<AssistantLangCode, Record<"unsafe" | "secret" | "illegal", string>>> = {
  hi: {
    unsafe: "Maaf kijiye, main aisi baaton par jawab nahi de sakta. Main sirf Videh messenger se judi madad kar sakta hoon.",
  ...
  en: {
    unsafe: "Sorry, I cannot help with that topic. I can only assist with Videh messenger features.",
  ...
  ta: {
    unsafe: "மன்னிக்கவும், இதற்கு நான் பதில் சொல்ல முடியாது. Videh messenger தொடர்பான உதவி மட்டும் செய்வேன்.",
  ...
  te: {
    unsafe: "క్షమించండి, దీనికి నేను సమాధానం చెప్పలేను. Videh messenger features లో మాత్రమే సహాయం చేస్తాను.",
  ...
  bn: {
    unsafe: "দুঃখিত, এ বিষয়ে আমি উত্তর দিতে পারি না। আমি শুধু Videh messenger-এ সাহায্য করতে পারi।",
...
export function safetyRefusal(
  ...
  return REFUSAL[lang]?.[category]
    ?? REFUSAL.hi?.[category]
    ?? "Maaf kijiye, main is par madad nahi kar sakta.";
}
